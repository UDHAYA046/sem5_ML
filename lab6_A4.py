# Lab06 A4 – Root Feature Detection with Binning Type Parameter
# Author: Udhaya
...
# --- Find root feature with binning type ---
def U_find_root_feature(U_df, U_target, U_binning_type="equal_width", U_bins=4):
...
def U_plot_ig_scores(U_ig_scores, U_title):
    ...
    U_series.plot(kind="bar", color="orange", edgecolor="black")
    U_plt.title(U_title)
...
if __name__ == "__main__":
    ...
    # Change binning parameters here
    U_binning_type = "equal_freq"  # "equal_width" or "equal_freq"
    U_bins = 4
    ...
    U_root_feat, U_root_ig, U_ig_scores = U_find_root_feature(U_df_all, U_target_col, U_binning_type, U_bins)

    print(f"[A4] Best root feature: {U_root_feat}")
    print(f"[A4] Information Gain: {U_root_ig}")
    U_plot_ig_scores(U_ig_scores, f"A4 – IG for All Features ({U_binning_type})")
