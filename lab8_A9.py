# ------------------------------------------------------------
# Lab 8 – A9 | XOR via 2–2–1 MLP (Sigmoid + CE gradients + Momentum)
# Author : S. Udhaya Sankari | BL.EN.U4CSE23150
# Goal   : Learn XOR with α = 0.05 (same as A1/A8), sigmoid activation.
# Stop   : When SSE <= 0.002 or after 1000 epochs.
# Saves  : C:\Users\Udhaya\sem5_ML\lab8_output_figures\U_A9_XOR_loss.png
# ------------------------------------------------------------
...
# ======================= Config ============================
U_SEED        = 482          # reproducible fast convergence
U_ALPHA       = 0.05         # learning rate (same as A1/A8)
...
U_V  = rng.uniform(-_lim_v, _lim_v, size=(2, 2))   # input->hidden
U_bh = np.zeros(2, dtype=float)                    # hidden bias

# 2 -> 1 output
_lim_w = U_xavier_limit(2, 1)
U_W  = rng.uniform(-_lim_w, _lim_w, size=(2, 1))   # hidden->output
...
for U_epoch in range(1, U_MAX_EPOCHS + 1):
    ...
    # ---- SSE for stop condition (as per lab’s rule)
    U_E   = U_Y - U_T                # (4x1)
...
print("\n=== A9: XOR via Backprop (2–2–1, sigmoid, CE+Momentum) ===")
...
plt.title("A9 – XOR via Backprop: SSE vs Epochs")
plt.xlabel("Epoch")
plt.ylabel("Sum-Square Error")
plt.grid(True, linestyle="--", linewidth=0.6)
U_plot_path = os.path.join(U_OUTPUT_DIR, "U_A9_XOR_loss.png")
